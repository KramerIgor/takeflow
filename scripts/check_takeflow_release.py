...
def check_docs_and_start_script() -> bool:
    checks = {
        ...
        "project_state_present": text_contains(PROJECT_ROOT / "docs" / "PROJECT_STATE.md", "Takeflow"),
        "agent_rules_present": text_contains(PROJECT_ROOT / "AGENTS.md", "Takeflow"),
...
def main() -> int:
    print("=== Takeflow release diagnostics ===")
    ...
    checks = [
        ...
        run_check(
            ...
            [
                ...
                "scripts/check_live_history_updates.py",
                "scripts/check_shutdown_flow.py",
                ...
                "scripts/check_product_backlog_pass.py",
                "scripts/check_runtime_api_settings.py",
                "scripts/check_retry_failed_button.py",
            ],
        ),
        run_check("ui_quality", [str(PYTHON), "-u", "scripts/check_ui_quality.py"]),
        ...
        run_check("product_backlog_pass", [str(PYTHON), "-u", "scripts/check_product_backlog_pass.py"]),
        run_check("runtime_api_settings", [str(PYTHON), "-u", "scripts/check_runtime_api_settings.py"]),
        run_check("failed_task_actions", [str(PYTHON), "-u", "scripts/check_retry_failed_button.py"]),
        ...
        run_check("live_history_updates", [str(PYTHON), "-u", "scripts/check_live_history_updates.py"]),
        run_check("shutdown_flow", [str(PYTHON), "-u", "scripts/check_shutdown_flow.py"]),
    ...
    if all(checks):
        print("RESULT=TAKEFLOW_RELEASE_DIAGNOSTICS_OK")
        return 0

    print("RESULT=TAKEFLOW_RELEASE_DIAGNOSTICS_FAILED")
    return 1
