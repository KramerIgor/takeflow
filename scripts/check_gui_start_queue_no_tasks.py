...
def main() -> int:
    print("=== GUI Start Queue no-task smoke test ===")

    client = TestClient(app)

    index = client.get("/")
    print("index_status_code=", index.status_code, sep="")
    print("contains_start_queue_button=", "Start Queue (paid, one task)" in index.text, sep="")
    print("contains_start_queue_endpoint=", "/start-queue-once" in index.text, sep="")
    ...
    response = client.post("/start-queue-once")

    queued_after = queued_tasks_count()

    print("start_queue_status_code=", response.status_code, sep="")
    ...
    if (
        ...
        and "Start Queue (paid, one task)" in index.text
        and "No queued tasks to process" in response.text
        and queued_after == 0
    ):
        print("RESULT=GUI_START_QUEUE_NO_TASKS_OK")
        return 0

    print("RESULT=GUI_START_QUEUE_NO_TASKS_FAILED")
    return 1
